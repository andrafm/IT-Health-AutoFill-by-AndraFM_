...
class OfficeForm(ctk.CTkFrame):
    def __init__(self, master, user_name, send_form, show_info, show_warning, show_error, accent="#3b82f6", card="#111214", on_open_toko=None, show_success=None):
        ...
        self.accent = accent
        self.card = card
        self.on_open_toko = on_open_toko
    ...
    def _build(self):
        ...
        ctk.CTkLabel(header, text="FORM OFFICE", font=("Segoe UI", 18, "bold"), text_color="white").pack(pady=(8, 0))
        ctk.CTkLabel(
            header,
        ...
        date_row.pack(fill="x", padx=12, pady=(12, 4))
        ...
        ctk.CTkLabel(card, text="Nama Komputer", font=("Segoe UI", 11, "bold")).pack(anchor="w", padx=12, pady=(10, 2))
        self.entry_kode = ctk.CTkEntry(card, width=200, height=28, font=("Segoe UI", 11), placeholder_text="Contoh: SB-NB001 / SB-PC001")
        self.entry_kode.pack(anchor="w", padx=12)

        self.target_var = ctk.StringVar(value="Office HO/Cabang")

        kondisi_row = ctk.CTkFrame(card, fg_color="transparent")
        kondisi_row.pack(fill="x", padx=14, pady=(12, 4))
        ctk.CTkLabel(kondisi_row, text="Perawatan Perangkat Keras", width=145, anchor="w", font=("Segoe UI", 11, "bold")).pack(side="left")
        ctk.CTkLabel(kondisi_row, text="", width=36).pack(side="left")
        ...
        self._add_status_row(card, "PC/Laptop", self.pc_var)
        self._add_status_row(card, "Keyboard", self.keyboard_var)
        self._add_status_row(card, "Mouse/Touchpad", self.mouse_var)
        self._add_status_row(card, "Monitor/LCD", self.monitor_var)
        self._add_status_row(card, "UPS/Battery", self.ups_var)

        btn_frame = ctk.CTkFrame(card, fg_color="transparent")
        btn_frame.pack(fill="x", padx=12, pady=(14, 10))
    ...
    def _collect_payload(self):
        ...
        return {
            ...
            "entry.385266849": "SBY",
            "entry.1885250742": self.target_var.get(),
            "entry.1590388624": self.entry_kode.get().strip(),
            "entry.1393941387": self.pc_var.get(),
            "entry.586962673": self.keyboard_var.get(),
            "entry.326258373": self.mouse_var.get(),
            "entry.96266344": self.monitor_var.get(),
            "entry.1969377734": self.ups_var.get(),
            "entry.1602601477": "Ya",
            "entry.1435005536": "Ya",
            "entry.263709496": "Ya",
            "entry.1181879015": "Ya",
    ...
    def preview(self):
        ...
        self.show_info("Preview Form Office", text)
    ...
    def _build_preview_text(self):
        ...
        return (
            ...
            f"Nama Komputer: {self.entry_kode.get() or '<kosong>'}\n"
            f"Target: {self.target_var.get()}\n"
            f"PC/Laptop: {self.pc_var.get()}\n"
            f"Keyboard: {self.keyboard_var.get()}\n"
            f"Mouse/Touchpad: {self.mouse_var.get()}\n"
            f"Monitor/LCD: {self.monitor_var.get()}\n"
            f"UPS/Battery: {self.ups_var.get()}"
        )
    ...
    def on_submit(self):
        ...
        if not kode:
            self.show_warning("Validasi", "Isi Nama Komputer terlebih dahulu")
            return
